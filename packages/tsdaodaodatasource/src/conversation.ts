...
export class ConversationProvider implements IConversationProvider {
  async deleteConversation(channel: Channel): Promise<void> {
    return axios.delete(
      `conversations/${channel.channelID}/${channel.channelType}`
    );
  }
  ...
  revokeMessage(message: Message): Promise<void> {
    return WKApp.apiClient.post(
      `message/revoke?channel_id=${message.channel.channelID}&channel_type=${message.channel.channelType}&message_id=${message.messageID}&client_msg_no=${message.clientMsgNo}`
    );
  }
  ...
  async syncMessages(
    channel: Channel,
    opts: SyncMessageOptions = new SyncMessageOptions()
  ): Promise<Message[]> {
    var messages = await this.syncGetMessages(channel, opts);
    return messages;
  }

  async syncGetMessages(
    channel: Channel,
    opts: SyncMessageOptions
  ): Promise<Message[]> {
    let messages = new Array<Message>();
    const limit = opts.limit || 15;
    var resp = await WKApp.apiClient.post(`message/channel/sync`, {
      limit: limit,
      channel_id: channel.channelID,
      channel_type: channel.channelType,
  ...
  async syncMessageExtras(
    ...
    let messageExtraObjs = await WKApp.apiClient.post(`message/extra/sync`, {
      channel_id: channel.channelID,
      channel_type: channel.channelType,
