    ...
    if (this.channel.channelType !== ChannelTypePerson) {
      ...
      if (this.subscribers && this.subscribers.length > 0) {
        for (const subscriber of this.subscribers) {
          ...
          const userChannel = new Channel(subscriber.uid, 1);
          const userChannelInfo = WKSDK.shared().channelManager.getChannelInfo(userChannel);
          subscriber.remark = subscriber.remark || userChannelInfo?.orgData.remark;
      ...
      this.routeData.subscribers = this.subscribers.filter(
        (s) => s.status === SubscriberStatus.normal
      );
